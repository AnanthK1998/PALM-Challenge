...
from segmentation_models.backbones import ResNeXt50,InceptionV3
...
def build_model(pretrained_weights=None):
    model= Unet('inceptionv3',input_shape=(256,256,3),encoder_weights='imagenet',freeze_encoder=True)
    model.compile(optimizer= Adam(lr = 1e-4),loss=jaccard_distance, metrics=[mean_iou,dice_coef])
...
model= build_model(pretrained_weights=None)
model_checkpoint = ModelCheckpoint('weights/unetInceptionV3_Jaccardloss_Adam.{epoch:02d}-{val_loss:.4f}-{val_mean_iou:.4f}.hdf5',mode='max',monitor='val_mean_iou',verbose=1, save_best_only=True)

reduce_lr = ReduceLROnPlateau(monitor='val_mean_iou',mode='max',factor=0.3, patience=10, min_lr=1e-9, verbose=1)
history=model.fit_generator(train_generator,steps_per_epoch=246,epochs=100,callbacks=[model_checkpoint,reduce_lr],validation_data=valGene,validation_steps= 6,verbose=1)
